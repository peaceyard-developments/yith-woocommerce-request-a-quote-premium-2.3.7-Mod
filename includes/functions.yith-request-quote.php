...
if ( ! function_exists( 'yith_ywraq_render_button' ) ) {
	...
	function yith_ywraq_render_button( $product_id = false ) {
		...
		if ( class_exists( 'YITH_WAPO_Type' ) && ( ! is_product() || in_array( $woocommerce_loop['name'], array( 'up-sells', 'related' ), true ) ) ) {
		...
		if ( $product->is_type( 'yith-composite' ) && ( ! is_product() || in_array( $woocommerce_loop['name'], array( 'up-sells', 'related' ), true ) ) ) {
			$template_button = 'add-to-quote-addons.php';
		}
	...
	function ywraq_get_order_status_tag( $status ) {
		switch ( $status ) {
			...
			default:
				...
				$status  = isset( $stauses[ 'wc-' . $status ] ) ? $stauses[ 'wc-' . $status ] : $status;
...
if ( ! function_exists( 'yith_ywraq_get_product_meta' ) ) {
	...
	function yith_ywraq_get_product_meta( $raq, $echo = true, $show_price = true ) {
		...
		} elseif ( is_object( $raq ) && isset( $raq->get_data()['variation_id'] ) && (int) $raq->get_data()['variation_id'] > 0 ) {
